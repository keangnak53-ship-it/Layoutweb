            ...
            <aside className={styles.sidebar}>
               ...
               <div className={styles.sidebarWidget}>
                  ...
                  {featuredProjects.map((project: Project) => (
                     <Link href={`/done-projects/${project.id}`} key={project.id} className={styles.projectSmallCard}>
                  ...
                  <ul className={styles.customList}>
                     {servicesData.filter((s: ServiceDetail) => s.slug !== slug).map((s: ServiceDetail) => (
                        <li key={s.slug} style={{marginBottom: '10px'}}>
                           <Link href={`/services/${s.slug}`} className="hover:text-red-600 transition-colors flex items-center gap-2">
